...
export default {
  computed: {
    ...
    $showSubSideBar () {
      ...
      const pageSidebarConfig = this.$page.frontmatter.sidebar || this.$localeConfig.sidebar || this.$themeConfig.sidebar
      const headers = this.$page.headers || []

      if ((pageSidebarConfig === 'auto' || pageSidebarConfig) && headers.length > 0) {
        return true
      }
      return false
    }
